...
@property(nonatomic,strong)DWVideoVisitorModel * visitorModel;// 访客信息数据
...
-(instancetype)initWithVisitorDict:(DWVideoVisitorModel *)visitor
{
    if (self == [super init]) {
...
-(void)verticalChangeUIAndData
{
    ...
    BOOL jump = self.visitorModel.isJump;
...
{
    if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:self.visitorModel.jumpURL]]) {
        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:self.visitorModel.jumpURL]];
    }
}
...
//解析数据
//-(void)proxyVisitorData:(NSDictionary *)visitorDict  DWVideoVisitorModel
-(void)proxyVisitorData:(DWVideoVisitorModel *)visitor
{
    if (!visitor) {
        return;
    }
    ...
    self.visitorModel = visitor;
    
    self.listArray = [[NSMutableArray alloc]init];
    
    for (DWVideoVisitorMessageModel * visitorMessageModel in visitor.visitorMessages) {
        NSMutableDictionary * mDict = [NSMutableDictionary dictionary];
        [mDict setValue:visitorMessageModel.visitorMes forKey:@"title"];
        [mDict setValue:visitorMessageModel.visitorTip forKey:@"placeholder"];
        [mDict setValue:@"" forKey:@"content"];
        [self.listArray addObject:mDict];
    }

    /*
    NSArray * visitorMessageArray = visitor.visitorMessages;
...
-(void)initUI
{
    ...
    self.titleLabel.text = self.visitorModel.title;
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        UIImage * headerImage = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:self.visitorModel.imageURL]]];
    ...
    if (!jump) {
        ...
        [leftButton setImage:[UIImage imageNamed:@"icon_play_return.png"] forState:UIControlStateNormal];
