...
-(void)setTextConfiguration
{
    ...
    //配置都是一样的，除了字体大小
    //大小
    self.fontName = self.subtitle.font;
    
    //编码格式
    self.encodeing = [self.subtitle.code isEqualToString:@"utf-8"] ? 0 : 1;
    
    //颜色
    self.color = [DWTools colorWithHexString:self.subtitle.color];
    //阴影颜色
...
-(void)asyncAnalysisData
{
    ...
    self.subtitleAnalysis1 = [[DWSubtitleAnalysis alloc]initWithSTRURL:[NSURL URLWithString:self.subtitle.url]];
    self.subtitleAnalysis1.encodeing = self.encodeing;
    [self.subtitleAnalysis1 parse];
    
    if (self.style == DWSubtitleStyleDouble) {
        self.subtitleAnalysis2 = [[DWSubtitleAnalysis alloc]initWithSTRURL:[NSURL URLWithString:self.subtitle2.url]];
